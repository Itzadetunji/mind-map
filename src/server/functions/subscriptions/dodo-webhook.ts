...
import { apiErrorResponse, StatusCodes } from "../../api/v1/utils";
...
export const APIRoute = createAPIFileRoute(
	"/api/v1/subscriptions/dodo-webhook",
)({
	POST: async ({ request }) => {
		const rawBody = await request.text();
		const webhookSecret = process.env.DODO_WEBHOOK_SECRET;

		console.log("Dodo webhook secret:", webhookSecret);
		...
		if (webhookSecret) {
			...
			if (!signatureHeader) {
				return apiErrorResponse(StatusCodes.UNAUTHORIZED, "Missing signature");
			}

			if (!verifySignature(rawBody, signatureHeader, webhookSecret)) {
				return apiErrorResponse(StatusCodes.UNAUTHORIZED, "Invalid signature");
			}
		}

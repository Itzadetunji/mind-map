...
const AUTOSAVE_DELAY = 1000; // 1 second debounce - gives time for multiple changes to batch
...
const ProjectPage = () => {
	...
	// Helper to extract only saveable node properties
	const getSaveableNodesJson = useCallback((nodeList: Node[]) => {
	...
	const getSaveableEdgesJson = useCallback((edgeList: Edge[]) => {
		return JSON.stringify(
			edgeList.map((e) => ({
				id: e.id,
				source: e.source,
	...
	useEffect(() => {
		if (project && isInitialLoadRef.current) {
			...
			lastSavedStateRef.current = {
				title: project.title,
				nodes: getSaveableNodesJson(nodes),
				edges: getSaveableEdgesJson(edges),
			};
			isInitialLoadRef.current = false;
		}
	}, [project, form, getSaveableNodesJson, getSaveableEdgesJson]);
	...
	const performAutosave = useCallback(async () => {
		...
		const currentNodesJson = getSaveableNodesJson(currentNodesRef.current);
		const currentEdgesJson = getSaveableEdgesJson(currentEdgesRef.current);
		...
		form,
		updateMutation,
		getSaveableNodesJson,
		getSaveableEdgesJson,
	]);
	...
	const handleNodesChange = useCallback(
		(nodes: Node[]) => {
			...
			// MindMap already filters for saveable changes, so just update and save
			currentNodesRef.current = nodes;
			triggerAutosave();
		},
		[triggerAutosave],

...
if (!RESEND_API_KEY) {
	console.error("Missing PROTOMAP_RESEND_API_KEY env var");
}

const resend = new Resend(RESEND_API_KEY ?? "");

// Types for common Supabase auth/user payload shapes.
// This keeps the function flexible depending on how you hook it up.
type AuthUserRecord = {
	id: string;
	email: string | null;
	raw_user_meta_data?: {
...
serve(async (req: Request): Promise<Response> => {
	...
	// Supabase sends data in different formats depending on the source:
	// - Database webhook (Edge Function): The record is sent directly as the body
	// - Auth hook: { record: { id, email, raw_user_meta_data, ... } }
	// - Database trigger: { new: { id, email, ... }, old: {...} }
	const payloadObj = payload as Record<string, unknown>;
	
	// Database webhook (Edge Function) format: The record IS the payload
	// Check if payload itself is a user record (has id and email)
	const isDirectRecord = payloadObj.id && payloadObj.email && typeof payloadObj.id === 'string';
	
	const record: AuthUserRecord | null =
		// Direct record (database webhook to Edge Function)
		(isDirectRecord ? (payloadObj as AuthUserRecord) : null) ??
		// Auth hook format
		(payloadObj.record as AuthUserRecord | null) ??
		// Database trigger format
		(payloadObj.new as AuthUserRecord | null) ??
		// Alternative auth hook format
		(payloadObj.user as AuthUserRecord | null) ??
		null;

	if (!record) {
		console.error("No user record found in payload. Full payload:", JSON.stringify(payloadObj, null, 2));
		return new Response(
			JSON.stringify({ 
				error: "Missing user record", 
				received: Object.keys(payloadObj) 
			}),
			{ 
				status: 400,
	...
	console.log("Extracted user record:", {
		id: record.id,
		email: record.email,
		hasRawMeta: !!record.raw_user_meta_data,
		hasUserMeta: !!record.user_metadata,
	});
	...
	if (!email) {
		console.error("User record has no email:", record);
		return new Response(
	...
	const from ="ProtoMap <hello@protomap.art>";
